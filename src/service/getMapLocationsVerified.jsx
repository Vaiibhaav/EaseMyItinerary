...
async function verifyLocationWithPlaces(loc) {
	...
	try {
		...
		// Very conservative: require at least one comma (name + address) or location
		const hasAddressLike =
			final.includes(",") && final.split(",").some(Boolean);
		const hasLocation = !!place.location;

		if (!hasAddressLike && !hasLocation) return null;
		...
		if (!address) {
			// allow it only if location exists AND displayName contains street-like data
			return hasLocation ? final : null;
		}
